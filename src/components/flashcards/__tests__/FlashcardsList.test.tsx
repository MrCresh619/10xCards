...
vi.mock('@/components/ui/pagination', () => ({
  ...
  PaginationLink: ({ children, onClick, isActive }: { children: React.ReactNode; onClick?: () => void; isActive?: boolean }) => (
    <button data-testid="pagination-link" onClick={onClick} data-active={isActive}>{children}</button>
  ),
  PaginationNext: ({ onClick }: { onClick?: () => void }) => (
    <button data-testid="pagination-next" onClick={onClick} aria-label="Następna strona">Następna</button>
  ),
  PaginationPrevious: ({ onClick }: { onClick?: () => void }) => (
    <button data-testid="pagination-previous" onClick={onClick}>Poprzednia</button>
  ),
  PaginationFirst: ({ onClick }: { onClick?: () => void }) => (
    <button data-testid="pagination-first" onClick={onClick}>Pierwsza</button>
  ),
  PaginationLast: ({ onClick }: { onClick?: () => void }) => (
    <button data-testid="pagination-last" onClick={onClick}>Ostatnia</button>
  ),
}));
...
vi.mock('../FlashcardForm', () => ({
  FlashcardForm: ({ onSubmit, initialValues }: any) => (
    <form
...
describe('FlashcardsList', () => {
  ...
  beforeEach(() => {
    ...
    (useFlashcards as any).mockReturnValue({
